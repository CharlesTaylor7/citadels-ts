...
import { DistrictData, DistrictName, DISTRICT_NAMES, getDistrictData } from "./districts";
 ...
 */
export function createPlayer(index: PlayerIndex, id: string, name: string): Player {
  return {
    index,
    id,
    name,
    ...
    countSuitForResourceGain(suit: CardSuit) {
      return this.city.filter(c => 
        c.name.data().suit === suit || c.name === "SchoolOfMagic"
      ).length;
    },
...
export class Game {
   ...
   */
  completeAction(playerIndex: PlayerIndex, cost: number, district: DistrictName): void {
      ...
      switch (this.activeTurn.type) {
        ...
        case "Draft": {
          ...
          if (draft.theaterStep) {
            ...
            if (this.players.length + 1 === this.characters.characters.length && 
        ...
        case "Call": {
          ...
          // Check for blackmail
          const blackmailerChar = this.characters.characters.find(c => c.role === "Blackmailer");
          if (blackmailerChar && blackmailerChar.player !== null) {
            const blackmailMarker = character.markers.find(m => 
  ...
  gainResourcesForSuit(suit: CardSuit): void {
    ...
    if (count > 0) {
      ...
      const activeRole = this.activeRoleMut();
      if (activeRole) {
        activeRole.logs.push(`${player.name} gains ${count} gold from ${suit} districts.`);
      }
    }
  }
        ...
        case "Call": {
          ...
          if (call.endOfRound) {
            ...
            if (player && activeRole) {
              ...
              const poorHouse = role !== "Witch" && 
                               player.gold === 0 && 
                               player.cityHas("PoorHouse");
              
              const park = role !== "Witch" && 
                          player.hand.length === 0 && 
                          player.cityHas("Park");
              
              if (poorHouse) {
                player.gold += 1;
                activeRole.logs.push(`${player.name} gains 1 gold from their Poor House.`);
              }
              
              if (park) {
                this.gainCards(2);
                activeRole.logs.push(`${player.name} gains 2 cards from their Park.`);
              }
              ...
              if (refund > 0) {
                this.alchemist = 0;
                player.gold += refund;
                activeRole.logs.push(`The Alchemist is refunded ${refund} gold spent building.`);
              }
            }
  ...
  callNext(): void {
    ...
    if (nextIndex !== undefined) {
      ...
      // Check for Emperor
      const emperor = this.characters.characters.findIndex(c => c.role === "Emperor");
      if (emperor >= 0) {
        this.activeTurn = {
          type: "Call",
          call: {
  ...
  endRound(): void {
    ...
    const heir = this.characters.characters.find(c => 
    ...
    if (heir && heir.player) {
      ...
      this.logs.push(`${heir.role}'s heir ${this.players[this.crowned[0]].name} crowned.`);
    }
